...
logpower_poly=polyfit(time,logpower,2);
logpower_poly_values=polyval(logpower_poly,time);
logpower_poly_values_separate=reshape(logpower,10,33);
time_move=reshape(time,10,33);

for i=1:33
    polyfit_derivative_logpower{i}=polyfit(time10,logpower_poly_values_separate(1:10,i),2);
